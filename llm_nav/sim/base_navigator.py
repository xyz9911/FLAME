...
class BaseNavigator:
    ...
    def __init__(self, opts):
        self.graph = GraphLoader(opts).construct_graph()
        self.states = list()
        self.actions = list()
        self.pano_path = list()
    ...
    def get_next_state(self, action):
        ...
        next_pano, next_heading = None, None
    ...
    def get_available_actions(self):
        ...
        forward_pano, forward_heading = self.get_next_state('forward')
        left_pano, left_heading = self.get_next_state('left')
        right_pano, right_heading = self.get_next_state('right')
        curr_node = self.graph.nodes[curr_pano]
        if len(curr_node.neighbors) == 1:
            available_actions.add('forward')
            return available_actions
        delta_left = get_relative_angle(left_heading, forward_heading)
        delta_right = get_relative_angle(forward_heading, right_heading)
        if left_heading == right_heading:
            available_actions.add('forward')
        elif delta_left > delta_right:
            available_actions.add('left')
            if len(curr_node.neighbors) == 3:
                if delta_right < 45:
                    available_actions.add('forward')
                else:
